...
import { Vendor, PaginatedResponse } from '../../models/master-data.models';
  ...
  template: `
    <div class="page-container">
      <header class="page-header">
        ...
        <a routerLink="new" class="btn btn-primary">+ New Vendor</a>
      </header>
      ...
      @if (vendorApi.loading()) {
        <div class="loading-indicator" role="status" aria-label="Loading vendors">
      ...
      @if (vendorApi.error()) {
        <div class="error-message" role="alert">
          <span>⚠️ {{ vendorApi.error() }}</span>
          <button class="btn btn-sm" (click)="loadVendors()">Retry</button>
        </div>
      }
      ...
      @if (!vendorApi.loading()) {
        <div class="table-container">
          <table class="data-table" aria-label="Vendors list">
            <thead>
              <tr>
                <th>Code</th>
                <th>Name</th>
            ...
            <tbody>
              ...
              @for (vendor of vendors(); track vendor.id) {
                <tr>
                  <td class="cell-code">{{ vendor.code }}</td>
                  <td>{{ vendor.name }}</td>
                  ...
                  <td class="cell-actions">
                    ...
                    <button
                      ...
                      [attr.aria-label]="'Deactivate ' + vendor.name"
                    >Deactivate</button>
                  </td>
                </tr>
              }
...
export class VendorListComponent implements OnInit {
  ...
  loadVendors(): void {
    ...
    this.vendorApi.getAll(params).subscribe({
      next: (response: PaginatedResponse<Vendor>) => {
        this.vendors.set(response.data);
        this.pagination.set(response.pagination);
      },
      error: (err) => {
        this.vendorApi.error.set(err?.error?.message || 'Failed to load vendors');
      },
    });
  }
  ...
  deleteVendor(vendor: Vendor): void {
    ...
    this.vendorApi.delete(vendor.id).subscribe({
      next: () => this.loadVendors(),
      error: (err) => {
        this.vendorApi.error.set(err?.error?.message || 'Failed to deactivate vendor');
      },
    });
  }
}
