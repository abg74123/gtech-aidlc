  ...
  template: `
    <div class="page-container">
      <header class="page-header">
        <h2 class="page-title">{{ isEditMode() ? 'Edit Customer' : 'New Customer' }}</h2>
        <a routerLink="/master-data/customers" class="btn btn-outline">← Back to List</a>
      </header>

      <!-- Loading -->
      @if (loadingItem()) {
        <div class="loading-indicator" role="status" aria-label="Loading customer">
      ...
      @if (!loadingItem()) {
        ...
        >
          ...
          <div class="form-actions">
            <a routerLink="/master-data/customers" class="btn">Cancel</a>
            <button
              type="submit"
              class="btn btn-primary"
              [disabled]="form.invalid || customerApi.loading()"
            >
...
export class CustomerFormComponent implements OnInit {
  ...
  private loadCustomer(id: string): void {
    ...
    this.customerApi.getById(id).subscribe({
      ...
      error: (err) => {
        this.errorMessage.set(err?.error?.message || 'Failed to load customer');
        this.loadingItem.set(false);
      },
    });
  }
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode() && this.customerId) {
      ...
      this.customerApi.update(this.customerId, dto).subscribe({
        next: () => this.router.navigate(['/master-data/customers']),
        error: (err) => {
      ...
      this.customerApi.create(dto).subscribe({
        next: () => this.router.navigate(['/master-data/customers']),
        error: (err) => {
